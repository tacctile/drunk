...
Last change: Bottom sheet behavior + popup simplification — (1) Scroll lock: `openPoiSheet` sets `document.body.style.overflow = 'hidden'`; `closePoiSheet` resets it to `''`; (2) Dismiss-only paths: Escape key no longer closes the poi sheet — only the X button (`data-sheet-close`) and tapping the dark overlay (`poiScrim`) dismiss it; (3) Bar pin popup: shows name + description only — no Open in Maps button; (4) Food pin popup: shows name + description only — no Open in Maps button; (5) Hotels pin popup and list cards unchanged — keep Open in Maps + Visit Website; (6) Food list below map: name + description only (hours field exists in data but has never been rendered in the list — confirmed clean); z-index already correct (`sheet-scrim: 60` > `nav-bar: 30`).
